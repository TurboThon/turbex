<script lang="ts">
  ...
  let loading = false;
    ...
    loading = true;
    try {
      const priv_key_and_password = await useAsync(() => crypt?.change_password(oldPassword, newPassword, $userStore!.privateKey));
...
<Modal title="Change password" bind:open={showModal} outsideclose on:close={clearFields}>
 <form>
    <p>Changing your password is a smooth process, we will re-encrypt your private keys with your new password.</p>
    ...
    {/if}
		<Label for="oldPassword" color={false ? "red" : undefined}>
      Old password
    </Label>
		<Input
			...
			class="mb-2"
		/>
		<Label for="newPassword" color={!newPasswordAreEquals ? "red" : undefined}>
      New password
    </Label>
		<Input
